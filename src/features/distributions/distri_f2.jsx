...
import { MapPin, Plus, Store, Trash2, Calendar, Filter, Calculator, RefreshCw, Save, ArrowRightCircle, Power, PowerOff, Edit2, X, Check, PlusCircle } from 'lucide-react'; 
...
export default function Distributions() {
  ...
  const [editItems, setEditItems] = useState([]); // Temporary items holder during edit
  ...
  const handleStartEdit = (dist) => {
      ...
      // Flatten structure for easier editing
  ...
  const handleSaveEdit = async () => {
      ...
      try {
          const payload = editItems.map(i => ({
              id: i.id, // Send ID if existing
              cake_id: i.cake_id,
              quantity_sent: parseInt(i.quantity_sent)
          }));
          ...
          alert("Data titipan berhasil diperbarui!");
          setEditingDistId(null);
          setEditItems([]);
          fetchData();
      } catch (err) {
...
// --- LOGIC: Auto-Fill Items ---
 ...
 const handleAddTemplateItem = () => {
  ...
  const handleReconcileAndRestock = async (item) => {
    ...
    try {
       const { error } = await supabase.rpc('reconcile_and_restock', {
          ...
          p_next_restock_qty: nextRestockQty
      });
      if (error) throw error;
      alert(`Berhasil!`);
      fetchData();
  ...
  const handleAddManualItem = () => {
    ...
    setItems([...items, { tempId: Date.now(), cake_id: manualCakeId, cake_name: cake.name, quantity: parseInt(manualQty), price: cake.price_per_piece, current_stock: cake.current_stock }]);
    setManualCakeId(''); setManualQty('');
  };
  
  useEffect(() => {
    if (selectedKioskId) {
      const kiosk = kiosks.find(k => k.id === selectedKioskId);
      if (kiosk && kiosk.kiosk_consignment_templates?.length > 0) {
        const autoItems = kiosk.kiosk_consignment_templates.map(t => {
           const masterCake = cakes.find(c => c.id === t.cake_id);
           return { tempId: Date.now() + Math.random(), cake_id: t.cake_id, cake_name: t.cakes?.name || 'Unknown', quantity: t.default_quantity, price: masterCake?.price_per_piece || 0, current_stock: masterCake?.current_stock || 0 };
        });
        setItems(autoItems);
      } else { setItems([]); }
    }
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6 font-sans">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <div><h1 className="text-2xl md:text-3xl font-bold text-gray-900">Manajemen Distribusi</h1><p className="text-gray-500 text-sm">Sistem Titipan, Tagihan & Auto-Restock</p></div>
          <div className="bg-white p-1 rounded-lg shadow-sm border border-gray-200 flex">
            <button onClick={() => setActiveTab('distribution')} className={`px-4 py-2 rounded-md text-sm font-medium ${activeTab === 'distribution' ? 'bg-indigo-600 text-white' : 'text-gray-600 hover:bg-gray-50'}`}>Distribusi</button>
            <button onClick={() => setActiveTab('kiosks')} className={`px-4 py-2 rounded-md text-sm font-medium flex items-center gap-2 ${activeTab === 'kiosks' ? 'bg-indigo-600 text-white' : 'text-gray-600 hover:bg-gray-50'}`}><Store size={16} /> Data Mitra</button>
          </div>
        </div>

        {activeTab === 'distribution' && (
          <div className="grid lg:grid-cols-12 gap-6 animate-fadeIn">
            {/* Form Input (Hidden logic same as before, keeping code minimal here for brevity but functional) */}
            <div className="lg:col-span-4">
              <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200 sticky top-6">
                <div className="border-b pb-3 mb-4"><h2 className="text-lg font-bold text-gray-800 flex items-center gap-2"><Plus size={18} className="text-indigo-600"/> Titipan Baru Manual</h2></div>
                <form onSubmit={handleCreateDistribution} className="space-y-4">
                  <div>
                    ...
                    <select value={selectedArea} onChange={e => { setSelectedArea(e.target.value); setSelectedKioskId(''); setItems([]); }} className="w-full px-3 py-2 bg-indigo-50 border border-indigo-100 rounded-lg"><option value="">-- Pilih Area --</option>{ROUTES.map(r => <option key={r} value={r}>{r}</option>)}</select>
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">2. Kios</label>
                    <select value={selectedKioskId} onChange={e => setSelectedKioskId(e.target.value)} className="w-full px-3 py-2 border rounded-lg" required><option value="">-- Pilih Kios --</option>{filteredKiosksForInput.map(k => <option key={k.id} value={k.id}>{k.name}</option>)}</select>
                  </div>
                  <div><label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Tanggal</label><input type="date" value={distDate} onChange={e => setDistDate(e.target.value)} className="w-full px-3 py-2 border rounded-lg" required /></div>
                  <hr className="border-gray-100 my-2" />
                  <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
                    <div className="flex justify-between items-center mb-2"><label className="block text-xs font-semibold text-gray-500 uppercase">3. Daftar Kue</label></div>
                    <div className="flex gap-2 mb-3">
                      <select value={manualCakeId} onChange={e => setManualCakeId(e.target.value)} className="flex-1 px-2 py-2 border rounded-lg text-sm bg-white"><option value="">+ Tambah Item</option>{cakes.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
                    ...
                    {items.map((item, idx) => (
                       <div key={item.tempId} className="flex justify-between items-center bg-white px-3 py-2 rounded text-sm border border-gray-200 shadow-sm mb-2">
                         ...
                         <div className="flex items-center gap-2"><span className="font-bold">{item.quantity}</span><button type="button" onClick={() => setItems(items.filter(i => i.tempId !== item.tempId))} className="text-red-400"><Trash2 size={14}/></button></div>
                       </div>
                    ))}
                  </div>
                  <button type="submit" disabled={isSubmitting || items.length === 0} className="w-full py-3 bg-indigo-600 text-white font-bold rounded-lg shadow hover:bg-indigo-700 flex justify-center items-center gap-2">{isSubmitting ? '...' : <><Save size={16}/> Simpan</>}</button>
                </form>
              </div>
            </div>
            ...
            <div className="lg:col-span-8 space-y-6">
              <div className="flex flex-col sm:flex-row justify-between items-center bg-white p-3 rounded-xl shadow-sm border border-gray-200 gap-3">
                 <div className="flex items-center gap-2 text-gray-600"><Filter size={18} /><span className="font-bold text-sm">Filter:</span></div>
                 <div className="flex gap-3 w-full sm:w-auto">
                    <input type="month" value={filterMonth} onChange={e => setFilterMonth(e.target.value)} className="py-2 px-3 border rounded-lg text-sm" />
                    <select value={filterArea} onChange={e => setFilterArea(e.target.value)} className="px-3 py-2 border rounded-lg text-sm bg-white"><option value="Semua">Semua Area</option>{ROUTES.map(r => <option key={r} value={r}>{r}</option>)}</select>
                 </div>
              </div>
                    ...
                    <div className={`px-6 py-4 border-b flex flex-col sm:flex-row justify-between sm:items-center gap-2 ${isFullyReconciled ? 'bg-green-50' : 'bg-white'}`}>
                      <div>
                        ...
                        <div className="flex items-center gap-4 mt-1 text-sm text-gray-500">
                           <span className="flex items-center gap-1"><Calendar size={14}/> {new Date(dist.distribution_date).toLocaleDateString('id-ID')}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                          {isEditing ? (
                              <div className="flex gap-2">
                                  <button onClick={handleSaveEdit} disabled={isSubmitting} className="flex items-center gap-1 bg-green-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow hover:bg-green-700">
                                      <Check size={14}/> Simpan
                                  </button>
                                  <button onClick={handleCancelEdit} disabled={isSubmitting} className="flex items-center gap-1 bg-gray-500 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow hover:bg-gray-600">
                                      <X size={14}/> Batal
                                  </button>
                              </div>
                          ) : (
                              !isFullyReconciled && (
                                  <button onClick={() => handleStartEdit(dist)} className="flex items-center gap-1 text-indigo-600 hover:bg-indigo-50 px-3 py-1.5 rounded-lg text-xs font-bold border border-indigo-200 transition-colors">
                                      <Edit2 size={14}/> Edit Titipan
                                  </button>
                              )
                          )}
                          ...
                          {/* Summary Badge */}
                          {isFullyReconciled ? (
                             <div className="text-right bg-green-100 px-3 py-1 rounded-lg border border-green-200 ml-2">
                                <div className="text-[10px] text-green-700 uppercase font-bold">Total</div>
                                <div className="text-sm font-bold text-green-700">{formatRupiah(totalBill)}</div>
                             </div>
                          ) : (
                             <div className="text-right bg-yellow-50 px-3 py-1 rounded-lg border border-yellow-200 ml-2">
                                <div className="text-[10px] text-yellow-700 uppercase font-bold">Status</div>
                                <div className="text-xs text-yellow-600 italic">Aktif</div>
                             </div>
                          )}
                      </div>
                    </div>
                                    ...
                                    {editItems.map((item, idx) => (
                                        <tr key={idx} className="border-b border-yellow-100 bg-white">
                                            <td className="px-4 py-2">
                                                {/* If ID exists (existing item), show name only (cannot change type). If new, dropdown */}
                                                {item.id ? (
                        ...
                        <table className="w-full text-sm text-left">
                            ...
                            {dist.distribution_items.map((item) => {
                                ...
                                <tr key={item.id} className="hover:bg-gray-50">
                                    <td className="px-5 py-3"><div className="font-medium text-gray-900">{item.cake_id?.name}</div></td>
                                    <td className="px-5 py-3 text-center text-gray-500 bg-gray-50 text-xs">{formatRupiah(item.price_at_distribution)}</td>
                                    <td className="px-5 py-3 text-center font-bold bg-indigo-50 text-indigo-800">{item.quantity_sent}</td>
                                    <td className="px-5 py-3 text-center">{hasWithdrawn ? <span className="font-bold text-green-700">{item.quantity_sold}</span> : '-'}</td>
                                    <td className="px-5 py-3 text-center text-red-500 font-medium">{hasWithdrawn ? item.quantity_damaged_at_location : '-'}</td>
                                    <td className="px-5 py-3 text-right font-bold text-gray-800">{hasWithdrawn ? formatRupiah(itemBill) : '-'}</td>
                                    <td className="px-5 py-3 text-center">
                                    {!hasWithdrawn ? (
                                        <button onClick={() => handleReconcileAndRestock(item)} className="bg-indigo-600 text-white px-3 py-1.5 rounded shadow-sm hover:bg-indigo-700 text-xs font-medium flex items-center gap-1 mx-auto transition-all transform hover:scale-105" title="Hitung setoran"><RefreshCw size={14}/> Setor</button>
                                    ) : (
                                        <span className="text-xs text-gray-400 flex items-center justify-center gap-1"><ArrowRightCircle size={14}/> Selesai</span>
                                    )}
                                    </td>
                                </tr>
                                );
              ...
              {filteredHistory.length === 0 && <div className="text-center py-16 bg-white rounded-xl border border-dashed border-gray-300"><Calculator className="mx-auto text-gray-200 mb-3" size={48}/><p className="text-gray-400 font-medium">Tidak ada data.</p></div>}
            </div>
          </div>
        )}
