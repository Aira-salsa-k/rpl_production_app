...
export default function Distributions() {
  ...
  try {
    ...
    console.log('Fetched distributions:', distData.data);
...
const handleEditItemChange = (idx, field, value) => {
    ...
    // KONVERSI KE NUMBER JIKA FIELD quantity_sent
...
const handleSaveEdit = async () => {
  ...
  try {
    // Format payload dengan benar - KIRIM ID untuk existing items
    const payload = editItems.map(i => {
      const item = {
        cake_id: i.cake_id,
        quantity_sent: i.quantity_sent
      };
      // Kirim ID untuk existing items, tidak kirim untuk items baru
      if (i.id) {
        item.id = i.id;
      }
      return item;
    ...
    console.log('Distribution ID:', editingDistId);
...
const handleBulkReconcile = async (dist) => {
  ...
  for (const item of dist.distribution_items) {
    ...
    if (damaged === null) return; // User cancelled
  ...
  try {
    ...
    alert(`Setoran berhasil! Total tagihan: ${formatRupiah(totalBill)}`);
    fetchData();
  } catch (err) {
...
// --- SINGLE ITEM RECONCILE (fallback) ---
const handleReconcileAndRestock = async (item) => {
  ...
  try {
    const { error } = await supabase.rpc('reconcile_distribution_bundle', {
      p_dist_id: item.distribution_id, // You might need to adjust this
      p_damaged_items: [{
        item_id: item.id,
        damaged: damaged,
        next_restock_qty: nextRestockQty
              ...
              {filteredHistory.map((dist) => {
                ...
                return (
                  <div key={dist.id} className={`bg-white border rounded-xl overflow-hidden shadow-sm transition-shadow ${isFullyReconciled ? 'border-green-200 ring-1 ring-green-100' : 'border-l-4 border-l-yellow-400 border-gray-200'}`}>
                    ...
                    {/* CARD HEADER */}
                    <div className={`px-6 py-4 border-b flex flex-col sm:flex-row justify-between sm:items-center gap-2 ${
                      isFullyReconciled ? 'bg-green-50' : 'bg-white'
                    }`}>
                        ...
                        {isFullyReconciled ? (
                          ...
                          <>
                            {dist.kiosks?.is_active? (
                              ...
                              /* NON ACTIVE BADGE */
                              <div className="text-right bg-red-50 px-3 py-1 rounded-lg border border-red-200 ml-2">
                                <div className="text-[10px] text-red-700 uppercase font-bold">Status</div>
                                <div className="text-xs text-red-600 italic">Non-Aktif</div>
                              </div>
                            )}
                          </>
                        )}
